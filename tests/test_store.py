...
class TestResolveSessionId:
    ...
    def test_same_full_id_is_ambiguous_without_platform(self, tmp_store: Store):
        session_id = "c1e8deadbeef"
        with tmp_store.open_session(session_id, platform="claude", cwd="/p") as w:
            w.append("x", 1)
    ...
    def test_qualified_short_prefix_searches_one_platform(self, tmp_store: Store):
        with tmp_store.open_session("c1e8claude", platform="claude", cwd="/p") as w:
            w.append("x", 1)
        with tmp_store.open_session("c1e8cursor", platform="cursor", cwd="/p") as w:
            w.append("x", 1)

        assert tmp_store.resolve_session_id("cursor:c1e8") == (
            "cursor",
            "c1e8cursor",
        )

    def test_qualified_prefix_ambiguous_within_platform(self, tmp_store: Store):
        with tmp_store.open_session("AB1111", platform="cursor", cwd="/p") as w:
            w.append("x", 1)
        with tmp_store.open_session("AB2222", platform="cursor", cwd="/p") as w:
            w.append("x", 1)
