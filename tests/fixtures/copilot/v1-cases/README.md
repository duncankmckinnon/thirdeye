...
session IDs use only the first 16 digest characters for readability, but the
full digest must be validated against the canonical home before reuse.  Hence
two homes cannot merge merely because their native session IDs match.
