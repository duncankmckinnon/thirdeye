...
def test_same_session_id_differs_across_platforms():
    calls = [
        lambda platform: trace_id_for_session(platform, "shared-session"),
        lambda platform: root_span_id_for_session(platform, "shared-session"),
        lambda platform: turn_span_id(platform, "shared-session", 7),
        lambda platform: chat_span_id(platform, "shared-session", "message"),
        lambda platform: tool_span_id(platform, "shared-session", "tool"),
    ]

    for derive in calls:
        assert derive("claude") != derive("cursor")
