...
def test_mint_uses_only_project_without_prompt(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("thirdeye.logfire_auth._authenticated_client", lambda: client)
...
def test_mint_wraps_project_list_errors(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("thirdeye.logfire_auth._authenticated_client", lambda: Boom())
