...
def _record(kind: str, source_id: str, payload: dict) -> SourceRecord:
    return {
        "source_id": source_id,
        "source_kind": kind,
...
def _capture_synthetic_batch(config: Config, paths: SourcePaths) -> str:
    ...
    records = [
        _record(
            "transcript",
            f"{paths['source_key']}/{NATIVE_ID}/prompt-1",
            {
                "type": "user.message",
                "id": "prompt-1",
                "timestamp": SOURCE_TS,
        ...
        _record(
            "transcript",
            f"{paths['source_key']}/{NATIVE_ID}/tool-1",
            {
                "type": "tool.execution_start",
                "id": "tool-1",
                "timestamp": SOURCE_TS,
                "data": {"toolName": "view", "arguments": {"path": "/fixture/workspace/alpha.txt"}},
            },
        ),
        _record(
            "hook",
            f"hook/{NATIVE_ID}/child-prompt-1",
            {
...
def test_store_lists_and_retains_raw_source_identity(tmp_path: Path) -> None:
    ...
    assert source_record["payload"]["type"] == "tool.execution_start"
...
def test_generic_cli_reads_search_raw_copilot_content(tmp_path: Path) -> None:
    ...
    tool_search = runner.invoke(main, ["search", "alpha.txt", "--platform", "copilot"], env=env)
