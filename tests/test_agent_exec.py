...
def test_streams_stdout_to_output_callback():
    ...
    with patch("shutil.which", return_value="/usr/bin/claude"), \
        ...
        run_agent_streaming(
            _make_harness(), "task", Path("/tmp"), output=captured.append
        )
...
def test_build_command_receives_composed_prompt():
    ...
    with patch("shutil.which", return_value="/usr/bin/claude"), \
        ...
        run_agent_streaming(
            _make_harness(), "my composed prompt", Path("/tmp"),
...
def test_thirdeye_home_triggers_pre_and_post_snapshot(tmp_path):
    ...
    with patch("shutil.which", return_value="/usr/bin/claude"), \
        ...
        run_agent_streaming(
            _make_harness(), "x", Path("/tmp"),
            output=lambda _: None, thirdeye_home=tmp_path,
        )
